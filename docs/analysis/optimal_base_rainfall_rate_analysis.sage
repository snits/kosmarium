...
global_rainfall_min = 1000  # mm/year (temperate minimum)
global_rainfall_max = 3000  # mm/year (tropical maximum)
target_rainfall_mm_year = 1200  # mm/year (temperate average)
...
mm_per_year_to_m_per_hour = 1e-3 / (365.25 * 24)  # mm/year → m/hour
...
print(f"Target rainfall rate: {target_rainfall_m_hour*1000:.3f} mm/hour")
print(f"Target rainfall rate: {target_rainfall_m_hour*24*1000:.3f} mm/day")
...
current_base_rate = 0.002  # m/hour
...
print(f"Current effective rate on test grid: {current_effective_rate_test*1000:.3f} mm/hour")
print(f"Current effective rate on test grid: {current_effective_rate_test*24*1000:.1f} mm/day")

# Calculate the absurd percentage increase this represents
reasonable_daily_rainfall_mm = 5.0  # mm/day - reasonable daily rainfall
current_daily_rainfall_mm = current_effective_rate_test * 24 * 1000
excess_percentage = (current_daily_rainfall_mm / reasonable_daily_rainfall_mm - 1) * 100
...
target_daily_mm = 3.0  # mm/day - conservative realistic target
target_effective_rate_m_hour = target_daily_mm / (24 * 1000)  # Convert to m/hour
...
for name, width, height in test_grids:
    ...
    daily_mm = effective_rate * 24 * 1000
    
    # Check if rainfall is in realistic range (0.5-10 mm/day)
    realistic = "YES" if 0.5 <= daily_mm <= 10.0 else "NO"
...
print(f"Percentage reduction: {(1 - optimal_base_rate_for_test / current_base_rate) * 100:.1f}%")

# Impact on the failing test case
print(f"\nImpact on 25x25 test grid:")
print(f"Current effective rate: {current_effective_rate_test:.6f} m/hour = {current_effective_rate_test*24*1000:.1f} mm/day")
new_test_effective_rate = optimal_base_rate_for_test * test_scaling_factor
print(f"New effective rate: {new_test_effective_rate:.8f} m/hour = {new_test_effective_rate*24*1000:.2f} mm/day")
print(f"Improvement: {current_effective_rate_test / new_test_effective_rate:.1f}x reduction")

# === CONTINENTAL SCALE VERIFICATION ===
print("\n=== CONTINENTAL SCALE VERIFICATION ===")

continental_effective_rate = optimal_base_rate_for_test * 1.0  # Reference scale factor = 1.0
continental_daily_mm = continental_effective_rate * 24 * 1000
continental_annual_mm = continental_daily_mm * 365.25
...
print(f"Realistic range check: {'YES' if 500 <= continental_annual_mm <= 1500 else 'NO'}")
...
print(f"Mass conservation: {'GOOD' if abs(mass_ratio - expected_mass_ratio) < 0.1 else 'POOR'}")
...
base_rate_multipliers = [0.5, 0.75, 1.0, 1.25, 1.5, 2.0]
...
for mult in base_rate_multipliers:
    ...
    test_daily = test_base_rate * test_scaling_factor * 24 * 1000
    cont_annual = test_base_rate * 1.0 * 24 * 1000 * 365.25
    
    status = "GOOD" if (1.0 <= test_daily <= 8.0) and (400 <= cont_annual <= 1800) else "BAD"
...
print()
print("VALIDATION:")
print(f"• 25x25 test grid: {new_test_effective_rate*24*1000:.2f} mm/day (realistic)")
...
print(f"Expected test grid daily rainfall: {new_test_effective_rate*24*1000:.2f} mm/day")
