...
impl SpatialUpdateTracker {
    ...
    pub fn mark_cell_changed(&mut self, x: usize, y: usize, change_magnitude: f32, _change_type: ChangeType) {
    ...
    fn propagate_to_neighbors(&mut self, center_x: usize, center_y: usize, propagated_magnitude: f32) {
...
impl OptimizedWaterFlowSystem {
    ...
    pub fn update_water_flow_selective(
        ...
        for (x, y) in self.update_tracker.active_cells() {
            ...
            let flow_magnitude = self.calculate_flow_at_cell(heightmap, water_depth, water_velocity, x, y);
            ...
            if elevation_change > 0.001 {
                self.update_tracker.mark_cell_changed(x, y, elevation_change, ChangeType::Terrain);
                any_changes = true;
            }
            
            if water_change > 0.001 {
                self.update_tracker.mark_cell_changed(x, y, water_change, ChangeType::Water);
                any_changes = true;
            }
        }
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                if water_depth[index] > 0.01 {
                    self.update_tracker.mark_cell_changed(x, y, water_depth[index], ChangeType::Water);
                }
            }
        }
    }
