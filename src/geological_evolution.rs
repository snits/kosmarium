...
use crate::sim::{WaterFlowSystem, WaterFlowParameters, WaterLayer};
use crate::climate::{TemperatureLayer, ClimateSystem};
...
impl GeologicalEvolution {
    ...
    pub fn evolve_terrain(
        ...
        if self.config.verbose_logging {
            println!("Starting geological evolution: {}x{} map, {} iterations", 
                     width, height, self.config.evolution_iterations);
        }
        
        // Create world scale for physics calculations
        let world_scale = WorldScale::new(10.0, (width as u32, height as u32), DetailLevel::Standard);
        ...
        let initial_total_elevation: f32 = initial_heightmap.iter()
        ...
        for iteration in 0..self.config.evolution_iterations {
            ...
            let pre_erosion_elevation: f32 = evolved_heightmap.iter()
                .flat_map(|row| row.iter())
                .sum();
            ...
            // Update statistics
            let post_erosion_elevation: f32 = evolved_heightmap.iter()
                .flat_map(|row| row.iter())
                .sum();
            ...
            if self.config.progress_interval > 0 && iteration % self.config.progress_interval == 0 {
                ...
                println!("Geological evolution progress: {:.1}% ({}/{})", 
                         progress, iteration, self.config.evolution_iterations);
            }
        }
        
        // Calculate final statistics
        let final_total_elevation: f32 = evolved_heightmap.iter()
            .flat_map(|row| row.iter())
            .sum();
        
        stats.total_iterations = self.config.evolution_iterations;
        stats.average_elevation_change = (final_total_elevation - initial_total_elevation).abs() / 
                                        (width * height) as f32;
        stats.river_network_length = self.calculate_river_network_length(&water_layer);
        stats.max_elevation_change = self.calculate_max_elevation_change(&initial_heightmap, &evolved_heightmap);
        
        if self.config.verbose_logging {
            println!("Geological evolution complete:");
            println!("  Average elevation change: {:.4}", stats.average_elevation_change);
                ...
                if water_layer.depth[y][x] > river_threshold {
                    ...
                    for dy in -1_i32..=1 {
                        for dx in -1_i32..=1 {
                            ...
                            if ny >= 0 && ny < water_layer.height() as i32 && 
                               nx >= 0 && nx < water_layer.width() as i32 {
...
mod tests {
    ...
    fn geological_evolution_modifies_terrain() {
        ...
        let has_changes = results.evolved_heightmap.iter()
        ...
        assert!(results.stats.total_erosion > 0.0, "Should have some erosion");
    }
}