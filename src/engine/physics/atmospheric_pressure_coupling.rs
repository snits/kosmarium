    ...
    pub fn from_atmospheric_conditions(
        ...
        for x in 0..width {
            for y in 0..height {
                ...
                let local_pressure = standard_pressure * elevation_factor + weather_pressure_variation;
                ...
                let evaporation_modifier = Self::calculate_evaporation_pressure_effect(pressure_ratio, temperature_kelvin);
        ...
        for x in 1..width-1 {
            for y in 1..height-1 {
                ...
                let dp_dx = (pressure_field[x+1][y] - pressure_field[x-1][y]) / (2.0 * grid_spacing_m);
                let dp_dy = (pressure_field[x][y+1] - pressure_field[x][y-1]) / (2.0 * grid_spacing_m);
        ...
        for x in 0..water_layer.width() {
            for y in 0..water_layer.height() {
                ...
                if current_depth > 1e-6 {
                    ...
                    let pressure_modified_evaporation = base_evaporation_rate * evaporation_modifier * self.pressure_influence;
                ...
                let (pressure_accel_x, pressure_accel_y) = pressure_effects.get_pressure_acceleration(x, y);
                ...
                if retention_modifier != 1.0 {
                    ...
                    let retention_change = current_depth * (retention_modifier - 1.0) * 0.01 * dt * self.pressure_influence;
...
mod tests {
    ...
    fn test_atmospheric_pressure_effects_calculation() {
        ...
        for x in 0..4 {
            for y in 0..4 {
                ...
                assert!(pressure > 80000.0 && pressure < 110000.0, 
                        "Pressure {:.1} Pa at ({}, {}) outside realistic range", pressure, x, y);
            }
        }
        ...
        for x in 0..4 {
            for y in 0..4 {
                ...
                assert!(evap_modifier > 0.5 && evap_modifier < 2.0,
                        "Evaporation modifier {:.3} at ({}, {}) outside reasonable range", evap_modifier, x, y);
            }
        }
        ...
        println!("  Pressure difference: {:.1} Pa", sea_level_pressure - mountain_pressure);
    }
    ...
    fn test_pressure_aware_water_flow_coupling() {
        ...
        let initial_total_water: f32 = (0..5).map(|x| (0..3).map(|y| water_layer.get_water_depth(x, y)).sum::<f32>()).sum();
        
        // Test with no pressure influence
        let mut water_layer_no_pressure = water_layer.clone();
        let mut flow_engine_no_pressure = FlowEngine::new(FlowAlgorithm::Conservation, 5, 3, &scale);
        ...
        let mut flow_engine_full_pressure = FlowEngine::new(FlowAlgorithm::Conservation, 5, 3, &scale);
        ...
        assert!(mountain_evap_modifier > valley_evap_modifier,
        ...
        let no_pressure_total: f32 = (0..5).map(|x| (0..3).map(|y| water_layer_no_pressure.get_water_depth(x, y)).sum::<f32>()).sum();
        let full_pressure_total: f32 = (0..5).map(|x| (0..3).map(|y| water_layer_full_pressure.get_water_depth(x, y)).sum::<f32>()).sum();
        
        // Should have some water loss due to pressure-enhanced evaporation
        assert!(full_pressure_total < no_pressure_total,
        ...
        println!("  Pressure-enhanced evaporation: {:.6}", no_pressure_total - full_pressure_total);
    }
}