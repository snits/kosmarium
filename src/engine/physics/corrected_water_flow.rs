...
use crate::engine::physics::water::{WaterLayer, Vec2};
...
impl CorrectedWaterFlowSystem {
    ...
    fn calculate_surface_gradient(
        ...
        let get_surface_elevation = |x: usize, y: usize| -> f32 {
            heightmap.get(x, y) + water.get_water_depth(x, y)
        };
        ...
        for y in 0..water.height() {
            for x in 0..water.width() {
                ...
                if flow_speed > erosion_flow_threshold && water_depth > erosion_depth_threshold {
                    let erosion_capacity = flow_speed * water_depth * self.base_system.parameters.erosion_strength;
                    ...
                    if current_sediment < erosion_capacity {
                        ...
                        let erosion_amount = (erosion_capacity - current_sediment).min(max_erosion_per_tick);
                        ...
                        let deposition_amount = (current_sediment - erosion_capacity) * self.base_system.parameters.deposition_rate;
                        let current_height = heightmap.get(x, y);
                        heightmap.set(x, y, current_height + deposition_amount);
                        water.sediment.set(x, y, current_sediment - deposition_amount);
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_surface_gradient_calculation() {
        ...
        let (dh_dx, dh_dy) = corrected_system.calculate_surface_gradient(&heightmap, &water, 25, 25);
