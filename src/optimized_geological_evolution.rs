...
use crate::spatial_partitioning::{OptimizedWaterFlowSystem, PerformanceStats, SpatialUpdateTracker, ChangeType};
use crate::cache_system::{CachedClimateSystem, CacheStats};
use crate::convergence_detection::{ConvergenceTracker, ConvergenceConfig, ConvergenceResult, ConvergenceStats};
...
impl OptimizedGeologicalEvolution {
    ...
    pub fn evolve_terrain_optimized(
        ...
        println!("Starting optimized geological evolution: {}x{} map", self.width, self.height);
        ...
        for iteration in 0..self.config.max_iterations {
            ...
            let _temperature_layer = self.cached_climate_system.get_cached_temperature_layer(&heightmap);
            ...
            if !water_changes_occurred && current_stats.active_cells == 0 {
                println!("No active changes detected - terminating early at iteration {}", iteration + 1);
                break;
            }
        }
        ...
        let performance_stats = self.calculate_performance_stats(peak_active_cells, min_active_cells);
        ...
        if self.config.enable_performance_logging {
            self.print_final_performance_report(&performance_stats, &convergence_stats, &cache_stats, total_time);
        }
    ...
    pub fn analyze_optimization_potential(&self, heightmap: &FlatHeightmap) -> OptimizationAnalysis {
    ...
    fn calculate_performance_stats(&self, peak_active: usize, min_active: usize) -> OptimizedPerformanceStats {
        let total_cells = self.width * self.height;
        let average_active = if !self.active_cells_history.is_empty() {
            self.active_cells_history.iter().sum::<usize>() as f32 / self.active_cells_history.len() as f32
        } else {
            0.0
        };
        ...
        let convergence_efficiency = self.convergence_tracker.get_convergence_stats()
        ...
        OptimizedPerformanceStats {
            ...
            minimum_active_cells: if min_active == usize::MAX { 0 } else { min_active },
    ...
    fn report_progress(&self, iteration: usize, convergence: &ConvergenceResult, spatial_stats: &PerformanceStats) {
        println!("\n=== Progress Report - Iteration {} ===", iteration + 1);
        println!("Convergence: {:.1}% complete", 
                 convergence.progress_info.as_ref().map(|p| p.progress_ratio * 100.0).unwrap_or(0.0));
    ...
    fn print_final_performance_report(
        ...
        println!("║   Total cells: {:<10} Peak active: {:<10}      ║", 
                 perf_stats.total_cells, perf_stats.peak_active_cells);
        println!("║   Avg active: {:<11.1} Min active: {:<11}      ║", 
        ...
        println!("║   Performance gain: {:<6.2}x                         ║", perf_stats.performance_gain);
        println!("║                                                       ║");
        println!("║ CONVERGENCE DETECTION PERFORMANCE:                   ║");
        println!("║   Iterations completed: {:<10}                       ║", conv_stats.total_iterations);
        println!("║   Converged at: {:<35} ║", 
                 conv_stats.convergence_iteration.map(|i| format!("iteration {}", i)).unwrap_or("did not converge".to_string()));
        println!("║   Iterations saved: {:<10}                           ║", conv_stats.iterations_saved(self.config.max_iterations));
        println!("║   Convergence efficiency: {:<6.1}%                   ║", perf_stats.convergence_efficiency * 100.0);
        println!("║                                                       ║");
        println!("║ CACHE SYSTEM PERFORMANCE:                            ║");
        println!("║   Cache hits: {:<10} Cache misses: {:<10}      ║", cache_stats.cache_hits, cache_stats.cache_misses);
        println!("║   Hit rate: {:<6.1}%                                 ║", cache_stats.hit_rate * 100.0);
        println!("║   Computational savings: {:<6.1}%                    ║", cache_stats.computational_savings() * 100.0);
        println!("║                                                       ║");
        println!("║ OVERALL PERFORMANCE:                                 ║");
        println!("║   Total execution time: {:<6.2}s                     ║", total_time.as_secs_f32());
        println!("║   Combined optimization gain: {:<6.2}x               ║", 
                 perf_stats.performance_gain * (1.0 + perf_stats.convergence_efficiency) * (1.0 + cache_stats.computational_savings()));
        println!("║                                                       ║");
        println!("╚═══════════════════════════════════════════════════════╝\n");
    }
...
mod tests {
    use super::*;
    use crate::scale::{WorldScale, DetailLevel};
    
    #[test]
    fn optimized_evolution_basic_functionality() {
        let width = 20;
        let height = 20;
        let world_scale = WorldScale::new(1.0, (width as u32, height as u32), DetailLevel::Standard);
    ...
    fn optimization_analysis() {
        let width = 50;
        let height = 50;
        let world_scale = WorldScale::new(5.0, (width as u32, height as u32), DetailLevel::Standard);
