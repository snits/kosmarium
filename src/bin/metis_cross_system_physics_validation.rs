...
impl MetisCrossSystemValidator {
    ...
    pub fn validate_all_physics_couplings(&mut self) {
        ...
        println!("Testing 8 physics couplings across domain scales: {:?} km", self.domain_sizes);
        println!();
    ...
    fn validate_single_domain(&self, domain_size_km: f64) -> MetisCrossSystemResults {
        ...
        let mut flow_engine = FlowEngine::new(FlowAlgorithm::Conservation, width as usize, height as usize, &scale);
        
        // Initialize atmospheric moisture system
        let mut atmospheric_moisture = AtmosphericMoistureSystem::new_for_scale(&scale, width as usize, height as usize);
        ...
        let (thermal_velocity_max, thermal_buoyancy_max, thermal_pressure_max) = 
            self.validate_thermal_circulation(&temperature_layer, &mut flow_engine, &climate_system, &scale);

        // 2. OROGRAPHIC PRECIPITATION VALIDATION  
        println!("    Testing orographic precipitation...");
        let (orographic_enhancement_max, vertical_velocity_max, condensation_rate_max) =
            self.validate_orographic_precipitation(&heightmap, &flow_engine, &mut atmospheric_moisture, &climate_system, &scale);

        // 3. MARITIME CLIMATE VALIDATION
        println!("    Testing maritime climate coupling...");
        let (maritime_velocity_max, pressure_anomaly_max, thermal_gradient_max) =
            self.validate_maritime_climate(&heightmap, &temperature_layer, &mut flow_engine, &scale);
    ...
    fn validate_thermal_circulation(
        ...
        use sim_prototype::engine::physics::thermal_circulation::{ThermalCirculationSystem, ThermalCirculationParameters};
        ...
        let mut atmospheric_pressure = AtmosphericPressureLayer {
            pressure: PhysicsGrid::new(scale.resolution.0 as usize, scale.resolution.1 as usize, 101325.0),
            pressure_gradient: PhysicsGrid::new(scale.resolution.0 as usize, scale.resolution.1 as usize, Vec2::new(0.0, 0.0)),
        };
    ...
    fn validate_conservation_laws(
        ...
        let (mass_error_percent, _conservation_valid) = atmospheric_moisture.validate_mass_conservation(total_moisture);
    ...
    fn create_test_terrain(&self, width: usize, height: usize) -> HeightMap {
        ...
        for x in 0..width {
            for y in 0..height {
                ...
                let mountain_height = (1.0 - (mountain_x * mountain_x + mountain_y * mountain_y).sqrt() * 2.0).max(0.0);
    ...
    fn analyze_scaling_violations(&self) {
        ...
        let thermal_velocities: Vec<f32> = self.results.iter().map(|r| r.thermal_velocity_max).collect();
        ...
        let orographic_enhancements: Vec<f32> = self.results.iter().map(|r| r.orographic_enhancement_max).collect();
        self.analyze_single_metric_scaling("Orographic Enhancement", &domain_sizes, &orographic_enhancements);
        
        // Maritime climate scaling analysis
        let maritime_velocities: Vec<f32> = self.results.iter().map(|r| r.maritime_velocity_max).collect();
        self.analyze_single_metric_scaling("Maritime Velocity", &domain_sizes, &maritime_velocities);

        println!();
    }

    /// Analyze scaling for single metric following successful statistical pattern
    fn analyze_single_metric_scaling(&self, metric_name: &str, domain_sizes: &[f64], values: &[f32]) {
        ...
        println!("    Correlation with domain size: {:.4} (should be ~0.0)", correlation);
        println!("    Scaling exponent: {:.4} (should be ~0.0)", scaling_exponent);
    ...
    pub fn print_results_summary(&self) {
        ...
        println!("Domain Size (km) | Grid Spacing (m) | Thermal V | Orographic E | Maritime V | Conservation");
        println!("----------------|-----------------|-----------|--------------|------------|-------------");
        
        for result in &self.results {
            println!("{:15.1} | {:15.1} | {:9.3} | {:12.3} | {:10.3} | {:11.2}%",
...
fn main() {
    ...
    println!("Use statistical analysis to identify physics violations and improvement opportunities.");
}