...
use sim_prototype::engine::physics::{
    thermal_circulation::{ThermalCirculationSystem, ThermalCirculationParameters},
    orographic_precipitation::{OrographicPrecipitationSystem, OrographicParameters},
    maritime_climate_coupling::{CoastalThermalEffects, MaritimAwareAtmosphereSystem},
    climate::ClimateSystem,
    flow_engine::{FlowEngine, FlowAlgorithm},
...
fn test_thermal_circulation_scaling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &domain_size_km in &domain_sizes {
        ...
        let mut pressure_layer = sim_prototype::engine::physics::climate::AtmosphericPressureLayer {
            pressure: sim_prototype::engine::core::PhysicsGrid::new(50, 50, 101325.0),
            pressure_gradient: sim_prototype::engine::core::PhysicsGrid::new(50, 50, 
        ...
        if let Some(effects) = thermal_system.get_effects() {
            ...
            println!("Domain {:6.0} km: Thermal velocity = {:.6} m/s", 
                     domain_size_km, velocity_magnitude);
        } else {
            thermal_velocities.push(0.0);
            println!("Domain {:6.0} km: No thermal effects calculated", domain_size_km);
        }
    }
    
    // Calculate scaling behavior
    let velocity_ratios: Vec<f32> = thermal_velocities.windows(2)
    ...
    let scale_invariance_achieved = velocity_ratios.iter()
...
fn test_orographic_precipitation_scaling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &domain_size_km in &domain_sizes {
        ...
        for x in 0..20 {
            for y in 0..20 {
                atmospheric_moisture.surface_moisture.set_humidity(x, y, 70.0); // 70% humidity
            }
        }
        ...
        println!("Domain {:6.0} km: Orographic enhancement = {:.3}x", 
                 domain_size_km, enhancement_factor);
    }
    
    // Calculate scaling behavior
    let enhancement_ratios: Vec<f32> = enhancement_factors.windows(2)
    ...
    let scale_invariance_achieved = enhancement_factors.iter()
        .all(|&factor| factor < 10.0); // Should not exceed reasonable limits vs previous 2500x
    
    println!("Realistic enhancement factors achieved: {}", scale_invariance_achieved);
    println!();
...
fn test_maritime_climate_scaling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &domain_size_km in &domain_sizes {
        ...
        println!("Domain {:6.0} km: Pressure anomaly = {:.1} Pa", 
                 domain_size_km, pressure_anomaly);
    }
    
    // Calculate scaling behavior
    let pressure_ratios: Vec<f32> = pressure_anomalies.windows(2)
    ...
    let scaling_improvement_achieved = pressure_ratios.iter()
        .any(|&ratio| ratio > 1.5); // Should increase vs. previous constant values
    
    println!("Scale-dependent pressure scaling achieved: {}", scaling_improvement_achieved);
    println!();
