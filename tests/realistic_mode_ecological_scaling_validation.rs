...
use sim_prototype::engine::physics::{
    ecosystem_feedback::{EcosystemFeedbackSystem, EcosystemFeedbackParameters, BiomeType},
...
fn test_realistic_mode_achieves_target_growth_rate() {
    ...
    assert!(scaling_error < 1e-10, 
        "Realistic mode scaling factor should be 2.5/3650 = {:.6}, but got {:.6} (error: {:.6})",
        expected_scaling_factor, actual_scaling_factor, scaling_error);
    
    // Test with realistic simulation timestep (0.1 hour)
    let dt_hours = 0.1;
    let base_growth_rate = 10.0; // kg/m²/day
    
    let scaled_rate = realistic_temporal_scaling.scale_ecosystem_growth_rate(base_growth_rate, dt_hours);
...
fn test_demo_mode_preserves_exact_behavior() {
    ...
    assert_eq!(scaled_rate, expected_demo_rate,
...
fn test_ecosystem_feedback_realistic_integration() {
    ...
    assert_eq!(ecosystem_system.get_temporal_mode(), TemporalMode::Realistic);
    ...
    ecosystem_system.biome_map_mut().set_biome(25, 25, BiomeType::Forest);
    ...
    assert!(biomass_change > 0.0, "Should have positive growth under optimal conditions");
    assert!(biomass_change < 0.01, "Growth should be much smaller than Demo mode (< 0.01 per timestep)");
    ...
    assert!(biomass_change < expected_relative_change * 10.0, 
        "Biomass change should reflect realistic scaling: got {:.9}, expected magnitude ~{:.9}",
        biomass_change, expected_relative_change);
}
...
fn test_ecological_process_coherence() {
    ...
    ecosystem_system.biome_map_mut().set_biome(10, 10, BiomeType::Grassland);
    ...
    assert!(biomass_change.abs() < 0.01,
...
fn test_realistic_mode_performance() {
    ...
    for _ in 0..iterations {
        let _scaled = realistic_system.temporal_scaling.scale_ecosystem_growth_rate(base_rate, dt_hours);
    }
    ...
    println!("Realistic mode scaling performance: {:.0} ops/sec", ops_per_second);
...
fn test_scaling_factor_mathematical_accuracy() {
    ...
    for &dt_hours in &test_timesteps {
        ...
        assert!(error < 1e-12, 
            ...
            dt_hours, expected_rate, scaled_rate, error);
    }
}
...
fn test_temporal_scaling_violation_fixed() {
    ...
    println!("   Annual accumulation: {:.3} kg/m²/year", annual_accumulation);
}