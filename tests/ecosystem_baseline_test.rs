...
use sim_prototype::engine::{
    ...
    physics::{
        ecosystem_feedback::{EcosystemFeedbackSystem, EcosystemFeedbackParameters},
...
fn test_capture_baseline_ecosystem_behavior() {
    ...
    let mut ecosystem_system = EcosystemFeedbackSystem::new(
    ...
    assert!(post_update_biomass >= initial_biomass, 
    ...
    assert!(post_update_biomass > 0.0, "Ecosystem should maintain positive biomass");
}
...
fn test_ecosystem_parameters_baseline() {
    ...
    assert_eq!(params.growth_rate, 10.0, "Growth rate should be 10.0 kg/m²/day (current hardcoded value)");
    
    println!("Baseline ecosystem parameters:");
    println!("  Growth rate: {} kg/m²/day", params.growth_rate);
    println!("  Base evapotranspiration: {} mm/day", params.base_evapotranspiration);
    println!("  Temperature moderation: {}°C per unit vegetation", params.temperature_moderation);
}