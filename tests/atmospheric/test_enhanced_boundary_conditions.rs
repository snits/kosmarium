...
use sim_protoype::engine::physics::atmosphere::{AtmosphericSystem, WindLayer};
...
fn test_pressure_gradient_scenario(atmospheric_system: &AtmosphericSystem, scale: &WorldScale) {
    ...
    let mut pressure_layer = sim_protoype::engine::physics::climate::AtmosphericPressureLayer::new(width, height);
