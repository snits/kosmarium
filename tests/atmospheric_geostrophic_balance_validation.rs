...
use kosmarium::engine::physics::atmosphere::{AtmosphericSystem, BoundaryType, WindLayer};
...
mod tests {
    ...
    fn test_phase_2_realistic_pressure_generation_validation() {
        ...
        let mut climate_system =
            kosmarium::engine::physics::climate::ClimateSystem::new_for_scale(&scale);
        ...
        let pressure_layer =
            climate_system.generate_pressure_layer(&temperature_layer, &heightmap, &scale);
        ...
        let unrealistic_percentage =
            (validation.unrealistic_wind_cell_count as f32 / total_cells as f32) * 100.0;
        assert!(
        ...
        println!(
            "  Total momentum magnitude: {:.1} m/s",
            conservation_validation.total_momentum_magnitude
        );
        println!("  {}", conservation_validation.diagnostic_summary);
        ...
        println!(
            "  - Pressure gradients in synoptic range ({:.6} Pa/m)",
            max_gradient_pa_per_m
        );
        println!(
            "  - Wind speeds dramatically reduced ({:.1} m/s max)",
            validation.maximum_wind_speed
        );
        println!(
            "  - Pressure-wind coupling established ({:.3} correlation)",
            validation.pressure_wind_correlation
        );
        println!(
            "  - Mass conservation improved ({:.1} m/s total momentum)",
            conservation_validation.total_momentum_magnitude
        );
