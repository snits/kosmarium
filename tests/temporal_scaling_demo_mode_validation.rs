...
use sim_prototype::engine::{
    core::{
        ...
        temporal_scaling::{TemporalScalingService, TemporalScalingConfig, TemporalMode}
    },
    physics::{
        ecosystem_feedback::{
            EcosystemFeedbackSystem, EcosystemFeedbackParameters, BiomeMap, BiomeType
        },
    ...
    fn is_identical(&self, other: &EcosystemState, tolerance: f64) -> bool {
        ...
        for x in 0..self.biomass_values.len() {
            ...
            for y in 0..self.biomass_values[x].len() {
                ...
                if diff > tolerance {
                    eprintln!("Biomass mismatch at ({}, {}): {} vs {} (diff: {})", 
        ...
        for x in 0..self.vegetation_density.len() {
            for y in 0..self.vegetation_density[x].len() {
                let diff = (self.vegetation_density[x][y] - other.vegetation_density[x][y]).abs() as f64;
                if diff > tolerance {
                    eprintln!("Vegetation density mismatch at ({}, {}): {} vs {} (diff: {})", 
        ...
        if total_biomass_diff > tolerance {
            eprintln!("Total biomass mismatch: {} vs {} (diff: {})", 
                      self.total_biomass, other.total_biomass, total_biomass_diff);
            return false;
        }
        
        let avg_vegetation_diff = (self.avg_vegetation_density - other.avg_vegetation_density).abs();
        if avg_vegetation_diff > tolerance {
            eprintln!("Average vegetation density mismatch: {} vs {} (diff: {})", 
...
fn create_test_setup() -> (EcosystemFeedbackSystem, TemperatureField, WaterLayer, SurfaceMoistureLayer, FlowEngine, WorldScale) {
    let width = 20;
    let height = 15;
    ...
    let ecosystem_system = EcosystemFeedbackSystem::new(
    ...
    for x in 0..width {
        for y in 0..height {
            ...
            let lake_water = if (x as i32 - width as i32/4).abs() < 3 && (y as i32 - height as i32/4).abs() < 2 { 0.6 } else { 0.0 }; // Small lake
    ...
    (ecosystem_system, temperature_field, water_layer, moisture_layer, flow_engine, world_scale)
}
...
fn test_capture_baseline_ecosystem_behavior() {
    let (mut ecosystem_system, mut temperature_field, mut water_layer, mut moisture_layer, flow_engine, world_scale) = create_test_setup();
    ...
    assert!(post_update_state.total_biomass > initial_state.total_biomass, 
           "Expected ecosystem growth in favorable conditions");
    
    println!("Baseline behavior captured:");
    println!("  Initial total biomass: {:.6}", initial_state.total_biomass);
    println!("  Post-update total biomass: {:.6}", post_update_state.total_biomass);
    println!("  Biomass growth: {:.6}", post_update_state.total_biomass - initial_state.total_biomass);
    println!("  Average vegetation density: {:.6}", post_update_state.avg_vegetation_density);
}
...
fn test_demo_mode_bit_identical_behavior() {
    let (mut baseline_system, mut baseline_temp, mut baseline_water, mut baseline_moisture, baseline_flow, world_scale) = create_test_setup();
    let (mut demo_system, mut demo_temp, mut demo_water, mut demo_moisture, demo_flow, _) = create_test_setup();
    ...
    for x in 0..baseline_system.biome_map().dimensions().0 {
        for y in 0..baseline_system.biome_map().dimensions().1 {
            ...
            demo_system.biome_map_mut().set_vegetation_density(x, y, vegetation);
        }
    }
...
fn test_demo_mode_performance_overhead() {
    ...
    println!("  Baseline duration: {:.3}ms", baseline_duration.as_secs_f64() * 1000.0);
    println!("  Demo mode duration: {:.3}ms", demo_duration.as_secs_f64() * 1000.0);
    println!("  Overhead: {:.2}%", overhead_percent);
    
    // Critical performance gate: <5% overhead
    assert!(overhead_percent < 5.0, 
           "Demo mode performance overhead ({:.2}%) exceeds 5% limit", overhead_percent);
    ...
    assert!(ticks_per_10s > 333.0, 
           "Performance ({:.0} ticks/10s) below minimum threshold (333 ticks/10s)", ticks_per_10s);
}