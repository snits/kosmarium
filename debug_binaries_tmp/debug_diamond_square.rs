...
fn main() {
    let seed = 12345;
    println!("Investigating Diamond-Square (0,0) mountain artifact with seed: {}", seed);
    ...
    let min_val = heightmap.iter().flatten().cloned().fold(f32::INFINITY, f32::min);
    let max_val = heightmap.iter().flatten().cloned().fold(f32::NEG_INFINITY, f32::max);
    ...
    println!("Is (0,0) the maximum? {}", heightmap[0][0] >= max_val - 1e-6);
    println!("Elevation percentile of (0,0): {:.2}%", 
             (heightmap[0][0] - min_val) / (max_val - min_val) * 100.0);
    
    // Check other corners for comparison
    println!("\nCorner comparison:");
    println!("Top-left (0,0): {:.6}", heightmap[0][0]);
    println!("Top-right (0,{}): {:.6}", width-1, heightmap[0][width-1]);
    println!("Bottom-left ({},0): {:.6}", height-1, heightmap[height-1][0]);
    println!("Bottom-right ({},{}): {:.6}", height-1, width-1, heightmap[height-1][width-1]);
    ...
    for (name, corners) in test_configs {
        ...
        let test_max = test_map.iter().flatten().cloned().fold(f32::NEG_INFINITY, f32::max);
        let corner_val = test_map[0][0];
        
        println!("{}: (0,0)={:.6}, max={:.6}, is_max={}", 
                 name, corner_val, test_max, corner_val >= test_max - 1e-6);
    }
}