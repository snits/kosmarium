...
fn test_map_size(name: &str, width: u32, height: u32) {
    ...
    println!("  Water conservation ratio: {:.6}", total_water_per_tick / reference_total_water);
    ...
    println!("  After evaporation per cell: {:.8}", net_water_per_cell_per_tick);
...
fn test_rainfall_scaling() {
    ...
    for (width, height, description) in test_cases {
        ...
        println!("  {} ({}x{}): scale_factor={:.6}, effective_rate={:.8}", 
...
fn test_evaporation_impact() {
    ...
    for (name, rate) in test_rates {
        ...
        println!("  {}: rate={:.8}, after_evap={:.8}, cleared={}",
...
fn test_water_simulation_tick() {
    ...
    for tick in 1..=10 {
        ...
        println!("  Tick {}: depth after rainfall={:.8}, after evaporation={:.8}",
                 tick, water_depth + large_map_rate, water_depth);
    ...
    if large_map_rate > evaporation_threshold * evaporation_rate {
        ...
        println!("  Theoretical steady state (if no clearing): {:.8}", steady_state);
    } else {
