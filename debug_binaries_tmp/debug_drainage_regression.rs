...
use sim_protoype::drainage::DrainageNetwork;
use sim_protoype::heightmap::HeightMap;
use sim_protoype::water::WaterLayer;
use sim_protoype::biome::{BiomeClassifier, BiomeType};
use sim_protoype::climate::ClimateSystem;
use sim_protoype::scale::{WorldScale, DetailLevel};
use sim_protoype::worldgen::{DiamondSquareGenerator, DiamondSquareConfig, TerrainGenerator};
...
fn test_small_map() {
    ...
    println!("  River cells: {} ({:.1}%)", stats.river_cells, stats.river_coverage() * 100.0);
    println!("  Major river cells: {} ({:.1}%)", stats.major_river_cells, stats.major_river_coverage() * 100.0);
    println!("  Depression cells: {} ({:.1}%)", stats.depression_cells, stats.lake_coverage() * 100.0);
    println!("  Sink cells: {} ({:.1}%)", stats.sink_cells, stats.sink_coverage() * 100.0);
    ...
    println!("  50th percentile (median): {:.1}", accumulation_samples[len / 2]);
    println!("  90th percentile: {:.1}", accumulation_samples[len * 9 / 10]);
...
fn test_larger_map() {
    ...
    println!("Drainage calculation time: {:.2}ms", drainage_time.as_millis());
    ...
    println!("  Tundra dominance: {:.1}% ({} cells)", tundra_percentage, tundra_count);
    println!("  Non-tundra cells: {} ({:.1}%)", non_tundra_biomes, non_tundra_biomes as f32 / total_cells * 100.0);
    ...
    for y in 0..heightmap.height().min(10) {
        for x in 0..heightmap.width().min(10) {
            ...
            if biome != BiomeType::Tundra && samples_found < 5 {
                ...
                let temperature = temperature_layer.get_current_temperature(x, y, climate_system.current_season);
                let water_depth = water_layer.get_water_depth(x, y);
                let accumulation = drainage_network.get_flow_accumulation(x, y);
                
                println!("  [{}, {}]: {:?} (elev: {:.3}, temp: {:.1}°C, water: {:.6}, flow_acc: {:.1})", 
