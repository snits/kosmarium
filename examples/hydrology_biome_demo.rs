...
fn main() {
    ...
    println!(
        "📊 Created test terrain ({}km domain, {}km/pixel)",
    ...
    for x in 1..5 {
        ...
        flow_engine
            .velocity_field
            .set_velocity(x, 1, Vec2::new(velocity, 0.0));
        flow_engine
            .velocity_field
            .set_velocity(x, 2, Vec2::new(velocity * 0.8, 0.0));
        flow_engine
            .velocity_field
            .set_velocity(x, 3, Vec2::new(velocity * 0.6, 0.0));
    }
    ...
    println!(
        "   Max velocity: {:.3} m/s",
        flow_engine.velocity_field.max_velocity_magnitude()
    );
    ...
    let (hydro_biomes, water_availability) = hydro_aware_classifier
    ...
    println!(
        "   Cells modified by hydrology: {} ({:.1}%)",
        differences,
        (differences as f32 / total_cells as f32) * 100.0
    );
    println!(
        "   Water-enhanced vegetation: {} ({:.1}%)",
        water_enhanced,
        (water_enhanced as f32 / total_cells as f32) * 100.0
    );
    ...
    println!(
        "   Cells with high water availability (>0.5): {}",
        high_availability_cells
    );
    ...
    println!(
        "   🌊 Water availability calculated from flow dynamics (residence time, watershed, intensity)"
    );
    ...
    if differences > 0 {
        println!(
            "   🎉 Hydrology influence successfully modified {} biome classifications",
            differences
        );
    }
